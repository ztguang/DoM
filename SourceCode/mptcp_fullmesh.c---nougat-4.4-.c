...
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr0 = %pI4\n", &fmp->remaddr4[0].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr1 = %pI4\n", &fmp->remaddr4[1].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr2 = %pI4\n", &fmp->remaddr4[2].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr3 = %pI4\n", &fmp->remaddr4[3].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr4 = %pI4\n", &fmp->remaddr4[4].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr5 = %pI4\n", &fmp->remaddr4[5].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr6 = %pI4\n", &fmp->remaddr4[6].addr);
printk(KERN_INFO "mptcp: create_subflow_worker: remaddr7 = %pI4\n", &fmp->remaddr4[7].addr);
...
//printk(KERN_INFO "mptcp: create_subflow_worker: locaddr = %pI4 --- remaddr = %pI4\n", &mptcp_local->locaddr4[i].addr, &rem4.addr);

		//== 判断 srcaddr, dstaddr 是否属于 某个网段 或 同一个 网段
		//== if (locaddr == 112.26.0.0/24) || ( locaddr/24 == remaddr/24 )
	//	if (((mptcp_local->locaddr4[i].addr.s_addr & netaddr1.sin_addr.s_addr) == netaddr2.sin_addr.s_addr)
	//		|| ((mptcp_local->locaddr4[i].addr.s_addr & netaddr1.sin_addr.s_addr)
	//		 == (rem4.addr.s_addr & netaddr1.sin_addr.s_addr)))
		//== netaddr1.sin_addr.s_addr, 即, inet_aton(), 可以使用 inet_addr() 代替
	//	if (((mptcp_local->locaddr4[i].addr.s_addr & inet_addr("255.255.255.0")) == inet_addr("112.26.0.0"))
	//		|| ((mptcp_local->locaddr4[i].addr.s_addr & inet_addr("255.255.255.0"))
	//		 == (rem4.addr.s_addr & inet_addr("255.255.255.0"))))
		//== Android 内核 中 没有 inet_aton(), inet_addr(), 直接 使用 IP 地址 的 十六进制表示
		//== "255.255.255.0" = 0x00ffffff, "112.26.0.0" = 0x00001a70

		//== if (locaddr == 112.26.0.0/24) || ( locaddr/24 == remaddr/24 )
	//	if (((mptcp_local->locaddr4[i].addr.s_addr & 0x00ffffff) == 0x00001a70)
	//		|| ((mptcp_local->locaddr4[i].addr.s_addr & 0x00ffffff)
	//		 == (rem4.addr.s_addr & 0x00ffffff)))
		//== if ( locaddr/24 == remaddr/24 )
		if ((mptcp_local->locaddr4[i].addr.s_addr & 0x00ffffff) == (rem4.addr.s_addr & 0x00ffffff))
			printk(KERN_INFO "mptcp: create_subflow_worker: locaddr = %pI4 --- remaddr = %pI4\n", &mptcp_local->locaddr4[i].addr, &rem4.addr);
		else
			goto next_subflow;
//----------------------------------------------------------------------
...
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr0 = %pI4\n", &mptcp_local->locaddr4[0].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr1 = %pI4\n", &mptcp_local->locaddr4[1].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr2 = %pI4\n", &mptcp_local->locaddr4[2].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr3 = %pI4\n", &mptcp_local->locaddr4[3].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr4 = %pI4\n", &mptcp_local->locaddr4[4].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr5 = %pI4\n", &mptcp_local->locaddr4[5].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr6 = %pI4\n", &mptcp_local->locaddr4[6].addr);
printk(KERN_INFO "mptcp: full_mesh_new_session: locaddr7 = %pI4\n", &mptcp_local->locaddr4[7].addr);
